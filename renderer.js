  ...
  try {
    if (data.includes('WAYPOINT,')) {
      ...
      if (parts.length >= 3) {
        ...
        addReceivedMessage(`WAYPOINT: CURRENT=${waypointData.current}, TOTAL=${waypointData.total}, LAST=${waypointData.lastCompleted}`, 'waypoint');
        return true;
      }
    }
  } catch (error) {
...
async function connectToPort() {
  ...
  try {
    const result = await ipcRenderer.invoke('connect-port', selectedPort, selectedBaud);
    portModal.hide();
    updateConnectionUI(true);
    addReceivedMessage(`Connected to ${selectedPort} at ${selectedBaud} baud`, 'connection');
...
async function sendCommand(commandCode) {
  try {
    ...
    addSentMessage(`Sent command: ${commandCode} - ${getCommandName(commandCode)}`);
  } catch (error) {
...
function startPeriodicUpdates() {
  setInterval(() => {
    ...
    if (timeSinceUpdate > 10 && connectionIndicator.classList.contains('connected')) {
...
ipcRenderer.on('serial-data', (event, data) => {
  ...
  // Try to parse as GPS, status, or waypoint data
  if (!processGpsData(data) && !processStatusData(data) && !processWaypointData(data)) {
    // Data already displayed as raw, no need to display again
  }
});
...
ipcRenderer.on('command-ack', (event, command) => {
  addSentMessage(`Command acknowledged: ${command} - ${getCommandName(command)}`, 'ack');
});
